(function ($) {
	...
	$window.on("scroll", function () {
		...
		if (fromTop > 400) {
			...
			$("header .header-sticky").removeClass("active");
		}
	});
	...
	if ($welcomePopup.length) {
		var popupDelay = 1500; // Delay in milliseconds before showing popup
		var showOncePerSession = true; // Set to false to show every page load

		function showPopup() {
			$welcomePopup.addClass('active');
			$body.css('overflow', 'hidden');
		}

		function hidePopup() {
			$welcomePopup.removeClass('active');
			$body.css('overflow', '');
		...
		$welcomePopup.on('click', function (e) {
			if (e.target === this) {
				hidePopup();
			}
		});
		...
		$(document).on('keydown', function (e) {
			if (e.key === 'Escape' && $welcomePopup.hasClass('active')) {
				hidePopup();
			}
		});

		// Show popup after delay (if not already shown in this session)
		if (!(showOncePerSession && sessionStorage.getItem('popupShown') === 'true')) {
	...
	$('#newsletterForm').on('submit', function (e) {
		...
		$.ajax({
			...
			success: function (res) {
				$msg.css('color', '#27ae60').text(res.message).fadeIn();
				$input.val('');
			},
			error: function (xhr) {
				var message = 'Đã xảy ra lỗi. Vui lòng thử lại.';
				if (xhr.status === 422 && xhr.responseJSON) {
					message = xhr.responseJSON.message || Object.values(xhr.responseJSON.errors).flat()[0];
				} else if (xhr.status === 429) {
					message = 'Bạn đã gửi quá nhiều yêu cầu. Vui lòng thử lại sau.';
				}
				$msg.css('color', '#e74c3c').text(message).fadeIn();
			},
			complete: function () {
